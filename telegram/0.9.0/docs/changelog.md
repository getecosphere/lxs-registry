...
## 0.9.0
- **Fix: file attachments larger than ~1.5 MB were rejected.** The domain
  advertised a 45 MB attachment limit, but the HTTP router kept axum's default
  2 MB request-body limit, so any base64-encoded attachment over ~1.5 MB failed
  with `HTTP 413 Failed to buffer the request body: length limit exceeded`
  before the handler ran. The request body limit is now raised to 64 MB, which
  comfortably fits a 45 MB file after base64 inflation (~60 MB). No API change;

...
- **Login links never confirm an account exists.** `POST
  ...
  immediately.
- **A signup auto-login is a provisional session, not a lock.** When
  `EMAIL_VERIFICATION_REQUIRED=false`, `register`/`register-with-profile` mint a
  session with `origin: "register"`. A real sign-in with the correct password
  supersedes it (the provisional session is revoked) instead of returning 409,
  so a user who loses the signup browser is never locked out of the account
  they just created. Only `origin: "login"` sessions block other sign-ins. Do
  not build UI that assumes the first `/auth/login` after signup must 409.

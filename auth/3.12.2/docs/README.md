...
## Sessions and single-session

One active session per account. A session minted by a sign-in
(`origin: "login"`) blocks any further sign-in with `409 ALREADY_EXISTS` until
it is logged out or expires. A session minted by the signup auto-login
(`origin: "register"`, only when `EMAIL_VERIFICATION_REQUIRED=false`) is
provisional: a real sign-in with the correct password supersedes it instead of
being rejected, so a user who loses the signup browser can always sign back in.

...
## 3.13.0 (2026-09-26)
- **"Remember me" on login.** `POST /api/auth/login` accepts an optional
  `remember` boolean. When true the session, JWT and response use the long
  lifetime (`JWT_EXPIRATION`, default 30 days); when false they use the short
  session lifetime (`SESSION_SHORT_MS`, default 12 hours). Signup,
  login-link and OIDC-client logins stay long-lived. The OIDC provider's
  sign-in form gained a matching checkbox and sets a persistent `eco_token`
  cookie only when it is ticked (session-only cookie otherwise).

## 3.12.2 (2026-09-26)
- IdP sign-in page: added the same **show/hide password ("eye") toggle** as
  auth-ui, and pinned the auth-ui stylesheet to `?v=7`.

## 3.12.1 (2026-09-26)
- **IdP sign-in page now matches `auth-ui`.** The provider's `/oidc/authorize`
  sign-in page uses the same markup, copy, fonts and `/static/auth-ui.css`
  stylesheet as `auth-ui`, and includes the same links (`Forgot password?`,
  `Create an account` → `/signup`, `← Back to homepage`), so a first-time
  visitor can register straight from the SSO screen.

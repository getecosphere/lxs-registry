...
- **Body params:**
  ...
  | `password` | string | yes |
  | `remember` | bool | no |
- **`remember`** selects the session lifetime: `true` → the long lifetime
  (`JWT_EXPIRATION`, default 30 days); `false` or omitted → the short session
  lifetime (`SESSION_SHORT_MS`, default 12 hours). Token, session row and
  `expiresIn` all use the same TTL.
...
- **Errors:**
  ...
  - 409 → `{"code":"ALREADY_EXISTS","message":"Already signed in on another device. Sign out from that device first.","timestamp":"..."}` when the account already has an active **Login** session (single active session per account — sign out or wait for expiry before signing in again). A provisional session minted by the signup auto-login does **not** block login: the real sign-in supersedes it.
...
- **Purpose:** Create an account and immediately issue a usable local session
  ...
  is left behind.
- **Session origin:** when email verification is disabled the minted session is
  *provisional* (`origin: "register"`). It works like any session, but a later
  real sign-in (`/auth/login`, login-link confirm, OIDC) supersedes it rather
  than being blocked with 409 — so a user who loses the signup browser can still
  sign in. Once a real sign-in happens, the strict single-active-session rule
  applies again.
- **Auth required:** no
- **Body params:**
  | Param | Type | Required |
...
### Admin user management (all `superadmin`-only)

- `GET /api/auth/admin/users?q=&status=active|deleted|all&limit=&offset=` —
  paginated accounts (`{ total, users: [UserDto] }`). Identity fields only;
  no credentials or hashes. Reads are audited.
- `POST /api/auth/admin/users/:id/deactivate` — soft-delete + revoke sessions.
  Refuses self and the last superadmin.
- `POST /api/auth/admin/users/:id/reactivate` — restore a soft-deleted account.
- `POST /api/auth/admin/users/:id/logout` — revoke every active session.
- `POST /api/auth/admin/users/:id/reset-password` — email a reset link; the
  admin never sets or sees a password.
- `POST /api/auth/admin/users/:id/verify-email` — mark the email verified.
- `PUT /api/auth/admin/users/:id/roles` (+ `/roles/add`) — replace/add roles.

Every mutation logs actor + target + action (NDJSON). There is deliberately
**no** admin "set password" endpoint: password changes go through the
reset-link flow, so an admin session can never directly take over an account.

## OIDC provider (IdP) mode

Enabled by `OIDC_PROVIDER_ENABLED=true`. Registered public clients are declared
in `OIDC_PROVIDER_CLIENTS` as `client_id|redirect_uri` entries joined by `;`.
All endpoints are public (browser redirects).

- `GET /api/auth/idp/oidc/authorize?client_id&redirect_uri&state&response_type=code`
  — if the `eco_token` cookie on this origin is valid, redirect immediately with
  `?code=...&state=...`; otherwise render a sign-in form.
- `POST /api/auth/idp/oidc/authorize` — form fields `client_id`, `redirect_uri`,
  `state`, `email`, `password`. Verifies credentials, mints an estate session
  (sets `eco_token`), and redirects with a code.
- `POST /api/auth/idp/oidc/token` — form fields `grant_type=authorization_code`,
  `code`, `client_id`, optional `redirect_uri`. Returns
  `{ access_token, token_type, id_token, expires_in, scope }`. Public clients:
  no `client_secret` required.
- `GET /api/auth/idp/oidc/userinfo` — `Authorization: Bearer <access_token>` →
  `{ sub, email, email_verified }`.
- `GET /api/auth/idp/.well-known/openid-configuration` — discovery document.

A client composes this IdP with `OIDC_ENABLED=true`, `OIDC_ISSUER` set to the
provider's issuer URL (clients append `/oidc/...`), `OIDC_CLIENT_ID`,
`OIDC_REDIRECT_URI`, and `OIDC_IDENTITY_VIA_USERINFO=true`.
